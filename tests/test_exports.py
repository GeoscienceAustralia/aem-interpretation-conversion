...
def test_gmtsddd_to_mdc(temp_sort_dir):
    temp_dir, sort_dir = temp_sort_dir
    export_dir = os.path.join(temp_dir, "export")
    boundary_file_content = "TYPE  Red  Green  Blue  Other\nA  10  20  30  1\nB 40 50 60 2\n"
    boundary_file_path = os.path.join(temp_dir, "colors.prn")
    create_prn_file(boundary_file_path, boundary_file_content)

    split_file_content = "TYPE  OVERAGE  UNDERAGE\nA 1 2\nB 3 4\n"
    split_file_path = os.path.join(temp_dir, "features.prn")
    create_prn_file(split_file_path, split_file_content)
    ...
    exports.gmtsddd_to_mdc(temp_dir, boundary_file_path, split_file_path, [101])
...
def test_gmtsddd_to_mdch(temp_sort_dir):
    ...
    exports.gmtsddd_to_mdch(temp_dir, boundary_file_path, split_file_path, [102])
